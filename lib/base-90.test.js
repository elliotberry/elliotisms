...
function roundTrip(value) {
  ...
  assert.match(encoded, PATH_SAFE)
...
describe("base-90", () => {
  ...
  it("does not emit path tokens that look like files", () => {
    const encoded = roundTrip(["my-bucket", "folder/sub/video.mp4"])
    assert.doesNotMatch(encoded, /\.[^/]+$/)
  })
  ...
  it("rejects invalid encoded input", () => {
    ...
    assert.throws(() => decode("AA."), /Invalid character/)
    assert.throws(() => decode("AA~"), /Invalid character/)
    assert.throws(() => decode("__"), /overflow/)
  })
})
